...
function Todo() {
  const [list, setList] = useState(JSON.parse(localStorage.getItem('activity')) || data);
  ...
  const handleSubmit = (e, input) => {
    ...
    setList((prev) => {
      return [...prev, { text: input, status: 'onProgress', id: `${currentId + 1}-${input}` }];
    });
  };
  ...
  return (
    <div className='relative z-10 flex h-auto max-w-xl px-10 mx-auto bg-yellow-3000 md:mx-auto'>
      <div className='w-full mt-20 text-left '>
        ...
        {/* Input */}
        <ListOfActivity list={list} filter={filter} checked={checked} removeOne={removeOne} handleDrag={handleDrag} />

        <Filter list={list} options={options} removeCompleted={removeCompleted} filter={filter} setFilter={setFilter} />
      </div>
    </div>
  );
}
